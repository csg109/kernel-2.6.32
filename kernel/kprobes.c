...
int __kprobes register_kprobe(struct kprobe *p)
{
	...
	/* 获取被探测点的地址，指定了symbol_name，则从kallsyms中获取；
	 * 指定了offset，则返回addr + offset */
	addr = kprobe_addr(p);
	if (!addr)
		return -EINVAL;
	p->addr = addr;
	...
	if (old_p) {
		 ...
		 * 并将所有的handler挂载到其链表上，由其负责所有handler函数的执行 */
	...
	/* 
	 * 分配特定的内存地址用于保存原有的指令
	 * 按照内核注释，被分配的地址必须 must be on special executable page on x86.
	...
	if (!kprobes_all_disarmed && !kprobe_disabled(p))
		__arm_kprobe(p); /* 将探测点的指令码修改为int 3指令 */
