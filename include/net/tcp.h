...
struct tcp_skb_cb {
	...
	__u32		end_seq;	/* SEQ + FIN + SYN + datalen	*//* 结束序列号，即SEQ + FIN + SYN + datalen */
	__u32		when;		/* used to compute rtt's	*//* 主要用来计算rtt, 记录了该数据包的第一次发送时间 */
	__u8		flags;		/* TCP header flags.		*//* tcp头的flag（比如syn，fin等),以下定义 */
...
struct tcp_congestion_ops {
	...
	unsigned long flags; 	/* 用来控制时间采样精度
				 * 如果设置为TCP_CONG_RTT_STAMP，则采用比较精密的时间采样函数，
				 * 否则采用jiffies转换函数
				 */
	...
	/* lower bound for congestion window (optional) *//* 计算并返回拥塞窗口最小值 */
	u32 (*min_cwnd)(const struct sock *sk);
	/* do new cwnd calculation (required) *//* 计算拥塞窗口 */
	void (*cong_avoid)(struct sock *sk, u32 ack, u32 in_flight);
	/* call before changing ca_state (optional) *//* 在拥塞状态改变时被调用 */
	void (*set_state)(struct sock *sk, u8 new_state);
	/* call when cwnd event occurs (optional) *//* 当有拥塞控制事件发生时被调用, 拥塞事件由tcp_ca_event定义 */
	void (*cwnd_event)(struct sock *sk, enum tcp_ca_event ev);
	/* new value of cwnd after loss (optional) *//* 撤销"缩小拥塞窗口"时被调用 */
	u32  (*undo_cwnd)(struct sock *sk);
	/* hook for packet ack accounting (optional) *//* 当发送方接收到ACK后，有段被确认时被调用。num_acked为此次ACK确认的段数 */
	void (*pkts_acked)(struct sock *sk, u32 num_acked, s32 rtt_us);
	/* get info for inet_diag (optional) *//* 提供给inet_diag的获取信息的接口 */
...
static inline __u32 tcp_current_ssthresh(const struct sock *sk)
{
	...
	if ((1 << inet_csk(sk)->icsk_ca_state) & (TCPF_CA_CWR | TCPF_CA_Recovery))
		...
		/* 当cwnd的3/4大于ssthresh，把ssthresh设成cwnd的3/4  */
