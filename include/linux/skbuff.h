...
struct sk_buff {
	...
	ktime_t			tstamp; /* 表示这个skb被接收/发送的时间 */
	struct net_device	*dev; 	/* 这个表示一个网络设备，当skb为输出时它表示skb将要输出的设备，
					 * 当接收时，它表示输入设备。 */

	unsigned long		_skb_dst; /* 要用于路由子系统。这个数据结构保存了一些路由相关信息 */
#ifdef CONFIG_XFRM
	struct	sec_path	*sp;
#endif
	/*
...
{
	if (likely(len <= skb_headlen(skb))) /* len小于skb的线性缓冲区，直接返回 */
		return 1;
	if (unlikely(len > skb->len)) /* len比skb中线性缓冲区和切换大小之和还要大，错误 */
		return 0;
	/* 到这里，线性缓冲区中还没有len大小的数据，调用__pskb_pull_tail()拉取数据 */
