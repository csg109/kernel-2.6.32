...
enum rq_cmd_type_bits {
	REQ_TYPE_FS		= 1,	/* fs request *//* 文件系统请求，用于与块设备之间的实际数据传输 */
...
struct request {
	...
	unsigned int cmd_flags; 	/* 请求通用标志, 由rq_flag_bits定义 */
	enum rq_cmd_type_bits cmd_type; /* 请求类型, 由rq_cmd_type_bits定义 */
	...
	sector_t __sector;		/* sector cursor *//* 要传送的下一个扇区号 */

	struct bio *bio; 		/* 请求中第一个没有完成传送操作的bio
					 * 通过bio的bi_next来指向下一个bio */
...
struct request_queue { /* 块IO请求队列 */
	/*
	 * Together with queue_head for cacheline sharing
	 */
	struct list_head	queue_head; 	/* 待处理请求的链表 */
	struct request		*last_merge; 	/* 指向队列中首先可能合并的请求描述符 
						 * 记录上一次被merge过的request，根据局部性原理， 
						 * 可以简单认为再访问了一个磁盘位置后，很大概率会连续访问，
						 * 所以在这儿记录了上一次被merge request。 
						 * 这样就不用在查找哪个request可以merge新到的bio了。
						 */
	struct elevator_queue	*elevator; 	/* I/O调度算法信息 */
	...
	request_fn_proc		*request_fn; 	/* 实现驱动程序的策略例程入口点的方法 
						 * 用于向队列添加新请求的标准接口，在内核期望驱动执行
						 * 某些工作时（如从底层设备读写），内核会自动调用该函数
						 */
	make_request_fn		*make_request_fn; /* 向请求链表添加请求
						   * 根据bio产生请求并发送给设备驱动程序。
		 				   * 对大多数设备，调用内核的标准函数：__make_request() 
						   */
	prep_rq_fn		*prep_rq_fn; 	/* 请求预备函数 */
	unprep_rq_fn		*unprep_rq_fn;
	unplug_fn		*unplug_fn; 	/* 用于拨出一个块设备时调用 */
	merge_bvec_fn		*merge_bvec_fn; /* 确定是否允许像一个现存的请求增加更多数据 */
	prepare_flush_fn	*prepare_flush_fn; /* 在预备刷出队列时会被调用 */
	softirq_done_fn		*softirq_done_fn; /* 在异步完成请求后，会被回调通知驱动程序请求已经完成 */
	 ...
	 */
	sector_t		end_sector; 	/* 最后分派的请求的末尾扇区 */
	struct request		*boundary_rq; 	/* 指向最后分派的请求 */
	...
	unsigned int		in_flight[2]; 	/* 请求队列中待处理的请求数 */
...
#define rq_data_dir(rq)		((rq)->cmd_flags & 1) /* 判断读写方向,0为读，1为写 */
...
struct block_device_operations { /* 块设备操作表 */
	int (*open) (struct block_device *, fmode_t); /* 打开块设备文件 */
	int (*release) (struct gendisk *, fmode_t);  	/* 关闭对块设备文件的最后一个引用 */
	int (*locked_ioctl) (struct block_device *, fmode_t, unsigned, unsigned long);
	/* 在块设备上发出ioctl系统调用, 使用大内核锁 */ 
	int (*ioctl) (struct block_device *, fmode_t, unsigned, unsigned long); 
	/* 在块设备上发出ioctl系统调用, 不使用大内核锁 */ 
