...
struct ctl_table 
{
	int ctl_name;			/* Binary ID *//* 唯一标识表项的一个整数 */
	const char *procname;		/* Text ID for /proc/sys, or zero *//* 在proc下显示的名称 */
	void *data; 			/* 对应内核中变量名称 */
	int maxlen; 			/* 主要用于字符串内核变量，以便在对该条目设置时，
					 * 对超过该最大长度的字符串截掉后面超长的部分 
					 */
	mode_t mode; 			/* 在proc下的访问权限
					 * 如果为目录， mode字段应当设置为 0555，否则无法访问它下面的条目
					 */
	struct ctl_table *child; 	/* 指向该目录条目下面的所有条目 */
	struct ctl_table *parent;	/* Automatically set */
	proc_handler *proc_handler;	/* Callback for text formatting */
					/* 指向对data成员实际进行读取和写入操作的一个函数；
					 * 它在通过/proc文件系统读写数据时被调用 */
