		    ...
		    __be16 ports[2])
{
	...
	if (ports[1] == svc->port) {
		...
		if (svc->port != FTPPORT)
			/* 从哈希表中获取已经保存的TEMPLATE的ip_vs_conn连接记录 */
		...
		if (!ct || !ip_vs_check_template(ct)) { /* 未找到TEMPALTE连接记录,则调用ipvs调度算法分配RS */
			/*
			 * No template found or the dest of the connection
			 * template is not available.
			 */
			/* 调用ipvs调度算法返回RS目标地址dest */
			...
			ct->timeout = svc->timeout; /* 设置连接记录的timeout时间 */
		} else { /* 对于已有TEMPLATE连接记录的，分配同一个dest */
