...
struct westwood {
	u32    bw_ns_est;        /* first bandwidth estimation..not too smoothed 8) *//* 经过一次平滑后的带宽值 */
	u32    bw_est;           /* bandwidth estimate *//* 最终的带宽值 */
	u32    rtt_win_sx;       /* here starts a new evaluation... *//* 采样周期的起始点，一个RTT后结束 */
	u32    bk;		 /* 在测量时间内确认的字节数 */
	u32    snd_una;          /* used for evaluating the number of acked bytes *//* 用于计算bk */
...
#define TCP_WESTWOOD_RTT_MIN   (HZ/20)	/* 50ms *//* RTT的最小值为50ms，带宽至少50ms才计算一次 */
#define TCP_WESTWOOD_INIT_RTT  (20*HZ)	/* maybe too conservative?! *//* RTT的初始值为20s，显然这个值太保守了 */
...
{
	if (w->reset_rtt_min) { /* 当发生超时后，最小RTT可能不再准确，需要更新 */
		w->rtt_min = w->rtt;
		w->reset_rtt_min = 0;
	} else
...
static inline void westwood_fast_bw(struct sock *sk)
{
	...
	westwood_update_window(sk); /* 更新带宽 */
...
static inline u32 westwood_acked_count(struct sock *sk)
{
	...
	if (w->cumul_ack > tp->mss_cache) {
		...
		if (w->accounted >= w->cumul_ack) { /* 表示此ACK为partial ACK */
...
static u32 tcp_westwood_bw_rttmin(const struct sock *sk)
{
	...
	/* 设置为 网络容量（不包括路由缓存中的），即 带宽 * RTTmin */
