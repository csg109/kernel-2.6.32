...
void sock_release(struct socket *sock)
{
	if (sock->ops) {
		...
		sock->ops->release(sock); /* TCP调用inet_release */
		sock->ops = NULL;
		module_put(owner);
	}
				       ...
				       struct msghdr *msg, size_t size, int flags)
{
	...
	return sock->ops->recvmsg(iocb, sock, msg, size, flags); /* TCP调用inet_recvmsg() */
}
...
SYSCALL_DEFINE2(listen, int, fd, int, backlog)
{
	...
	if (sock) {
		...
		if (!err)
			err = sock->ops->listen(sock, backlog); /* 即inet_listen() */
		...
		int __user *, upeer_addrlen, int, flags)
{
	...
	err = -ENFILE;
	if (!(newsock = sock_alloc())) /* 创建一个新的inode和socket */
		goto out_put;
	 ...
	 * 接下来调用inet_accept()。
	 */
	err = sock->ops->accept(sock, newsock, sock->file->f_flags); /* 调用inet_accept() */
	if (err < 0)
		goto out_fd;
