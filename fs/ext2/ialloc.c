 ...
 */
void ext2_free_inode (struct inode * inode) /* 删除一个索引节点 */
{
...
struct inode *ext2_new_inode(struct inode *dir, int mode)
{
	...
	sb = dir->i_sb;
	inode = new_inode(sb); /* 分配一个新的VFS索引节点对象 */
	if (!inode)
	...
	sbi = EXT2_SB(sb);
	es = sbi->s_es;
	if (S_ISDIR(mode)) { /* 如果要创建的是目录 */
		if (test_opt(sb, OLDALLOC))
			group = find_group_dir(sb, dir);
		else
			group = find_group_orlov(sb, dir); /* 为目录找到一个合适的块组 */
	} else 
		group = find_group_other(sb, dir); /* 如果不是目录，在有空闲节点的块组中分配 */
	...
	for (i = 0; i < sbi->s_groups_count; i++) {
		...
		brelse(bitmap_bh);
		bitmap_bh = read_inode_bitmap(sb, group); /* 获得该块组的索引节点位图 */
		if (!bitmap_bh) {
			err = -EIO;
			goto fail;
		}
...
got:
	...
	le16_add_cpu(&gdp->bg_free_inodes_count, -1); /* 减小描述符空闲Inode字段 */
	if (S_ISDIR(mode)) {
		if (sbi->s_debts[group] < 255)
			sbi->s_debts[group]++;
		le16_add_cpu(&gdp->bg_used_dirs_count, 1); /* 如果是目录则增加描述符已有目录字段 */
	} else {
	...
	ext2_preread_inode(inode); /* 从磁盘读入包含该索引节点的块，存入页高速缓存 */
	return inode;
