...
/* 返回分区中块组号为block_group对应的块组描述符，
 * 并将该块组描述符对应的块的缓冲区通过参数bh返回 */
struct ext2_group_desc * ext2_get_group_desc(struct super_block * sb,
					     unsigned int block_group, /* 块组号 */
					     struct buffer_head ** bh)
{
	...
	if (block_group >= sbi->s_groups_count) { /* 块组号大于快组数，出错 */
	...
	group_desc = block_group >> EXT2_DESC_PER_BLOCK_BITS(sb); /* 表示块组号整除每块中组描述符数，计算出该组的组描述符在组描述符表中的哪一块 */
	offset = block_group & (EXT2_DESC_PER_BLOCK(sb) - 1); /* 块组号与每块中组的描述符数进行与运算，得到这个组描述符具体是该块中第几个描述符 */
	if (!sbi->s_group_desc[group_desc]) { /* 找不到存放该块组描述符对应的块 */
		ext2_error (sb, "ext2_get_group_desc",
		    ...
		    unsigned long *count, int *errp)
{
	...
	*errp = -ENOSPC;
	...
	if (!ext2_has_free_blocks(sbi)) { /* 检查是否有空闲块 */
		*errp = -ENOSPC;
		goto out;
	}
	...
	if (goal < le32_to_cpu(es->s_first_data_block) ||
		...
		/* 如果目标块不在有效范围内，选择文件系统的第一个数据块为新目标 */
		goal = le32_to_cpu(es->s_first_data_block);
	group_no = (goal - le32_to_cpu(es->s_first_data_block)) /
			EXT2_BLOCKS_PER_GROUP(sb); 	/* 计算目标块所在的块组号 */
	goal_group = group_no;
retry_alloc:
	...
	free_blocks = le16_to_cpu(gdp->bg_free_blocks_count); /* 块组内空闲块的数目 */
	/*
	 * if there is not enough free blocks to make a new resevation
	 * turn off reservation for this allocation
	 */
	/* 如果启用了预分配，但空闲空间不够，那么关闭预分配 */
