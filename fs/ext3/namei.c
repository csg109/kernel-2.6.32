 ...
 */
/* @dir: 父目录的inode，
 * @dentry: 新创建的目录项，但是尚未与一个磁盘上的inode相关
 */
...
retry:
	/* 取得原子操作描述符,是journal_start()的封装 */
	handle = ext3_journal_start(dir, EXT3_DATA_TRANS_BLOCKS(dir->i_sb) +  /* 16 + 8 + 3 + 2*25 = 77 */
	...
	if (IS_DIRSYNC(dir))
		handle->h_sync = 1; 	/* 设置同步 */

	/* 在目录dir 所在的块组中分配一个新的inode，
	...
	if (!IS_ERR(inode)) {
		...
		/* 根据ext3的三种日志模式设置相对应的address_space_operations */
		ext3_set_aops(inode);
		/* 将新分配的inode与目录项关联起来，这样通过目录就可查找到文件 */
		err = ext3_add_nondir(handle, dentry, inode);
	}

	/* 关闭原子操作描述符,是journal_stop()的封装 */
