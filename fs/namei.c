		...
		int open_flag, int mode, int acc_mode)
{
	...
	int count = 0;
	int will_truncate;
	int flag = open_to_namei_flags(open_flag);  /* 改变参数flag的值，具体做法是open_flag+1 */

	if (!acc_mode) 	/* 设置访问权限 */ 
		acc_mode = MAY_OPEN | ACC_MODE(flag);

	/* O_TRUNC implies we need access checks for write permissions */
	if (flag & O_TRUNC) 	  /* 根据 O_TRUNC标志设置写权限 */
		acc_mode |= MAY_WRITE;
	...
	if (!(flag & O_CREAT)) { /* 如果不是创建文件 */
		...
		error = do_path_lookup(dfd, pathname, 	/* 查找文件 */
